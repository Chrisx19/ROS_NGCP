...
rotateLoop = PID_Controller (300, 600, 300, False)
    ...
    if Locked_On_Block:
      ...
      for Index in range (0, Count):
        if Blocks[Index].m_index == Locked_Block_Index:
          ...
          area = Blocks[Index].m_width * Blocks[Index].m_height
          ...
          Pan_Offset += Pan_PID_Controller.Command - PIXY_RCS_CENTER_POSITION

          servo = mapfloat(int(Pan_PID_Controller.Command), 0, 1000, -700, 700)

          if (servo > dead_zone and servo < dead_zone):
            servo = 0
          ...
          print('Val%d' % int(Pan_PID_Controller.Command), 'Area = %d' % area)
    else:
